...
void process_tmsg(int active_planes, int num_airports, int msg_queue_id, bool termination_requested);
...
int main() {
    ...
    msg_queue_id = msgget(MSG_KEY, 0666 | IPC_CREAT);
    if (msg_queue_id == -1) {
        perror("msgget");
        exit(1);
    }
    ...
    while (1) {
        ...
        if (!termination_requested) {
            process_amsg(a_msg, msg_queue_id); // Pass msg_queue_id
            active_planes++;
        }
...
if (termination_requested) {
        ...
        if (!termination_requested && msgrcv(msg_queue_id, &termination_msg, sizeof(struct p_message), TERMINATION_MSG_TYPE, IPC_NOWAIT) != -1) {
            // Process termination message
            termination_requested = true; // Set termination flag
        }
   
        process_tmsg(active_planes, num_airports, msg_queue_id, termination_requested); 
    }
